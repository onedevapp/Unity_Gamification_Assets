...
namespace OneDevApp
{
    ...
    public class Pagination : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        [Tooltip("How fast will page changes automatically")]
        [Range(4f, 30f)]
        public float changeScreenSaverInSec = 12f;
        ...
        void Update()
        {
            ...
            if (_pageSelectionImages != null && _pageSelectionImages.Count > 0)
            {
                elapsedTime += Time.deltaTime;

                if (elapsedTime >= changeScreenSaverInSec)
                {
                    elapsedTime -= changeScreenSaverInSec;

                    NextScreen();
                    if ((_currentPage + 1) == _pageSelectionImages.Count)
                        _currentPage = -1;
                }
            }
        }
        ...
        void GetFiles()
        {
            var info = Application.persistentDataPath + "/ScreenSaver/";
            ...
            foreach (string file in Directory.GetFiles(info))
            {
                ...
                GameObject pageSelectionIconsGO = Instantiate(paginationDotPrefab) as GameObject;
                Image pageSelectionItem = pageSelectionIconsGO.GetComponent<Image>();
                pageSelectionItem.sprite = unselectedPage;

                GameObject rawImageItemGO = Instantiate(rawImagePrefab) as GameObject;
                RawImage imageItem = rawImageItemGO.GetComponent<RawImage>();
                //rawImageItemGO.GetComponent<RectTransform>().sizeDelta = new Vector2(Screen.width, Screen.height);
                ...
                if (isLoaded)
                {
                    imageItem.texture = sampleTexture;
                }

                rawImageItemGO.transform.SetParent(_container, false);

                pageSelectionItem.transform.SetParent(pageSelectionIcons, false);
            }
        }
